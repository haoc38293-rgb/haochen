...
async function playWord(word:string,setToast:(s:string)=>void){
  try{if(LOCAL_AUDIO.has(word)){await new Audio(`/audio/${word}.mp3`).play();return}const r=await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`);const d=await r.json();const url=d?.flatMap((e:{phonetics?:{audio?:string}[]})=>e.phonetics||[]).find((p:{audio?:string})=>p.audio)?.audio;if(!url)throw new Error();await new Audio(url).play()}catch{setToast("这个词暂无真人录音");setTimeout(()=>setToast(""),1600)}
}
...
export default function Home(){
  ...
  useEffect(()=>{fetch("/gaokao-vocab.json").then(r=>r.json()).then((d:Vocab[])=>{setWords(d);setActive(d[0]?.w||"achieve")});try{setKnown(JSON.parse(localStorage.getItem("yujing-known")||"[]"));setSaved(JSON.parse(localStorage.getItem("yujing-saved")||"[]"));setReviewPlan(JSON.parse(localStorage.getItem("yujing-review")||"{}"))}catch{}},[]);
  ...
  return <main className="app-shell">
    ...
    {view==="review"&&<Dictation words={dueWords} plan={reviewPlan} onResult={finishReview} onAudio={async w=>{setActive(w);await playWord(w,setToast)}}/>}
