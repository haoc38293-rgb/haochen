...
type View="learn"|"reading"|"review"|"book"|"library";
...
export default function Home(){
  ...
  const [article,setArticle]=useState(0); const [query,setQuery]=useState(""); const [limit,setLimit]=useState(60); const [toast,setToast]=useState(""); const [loadingAudio,setLoadingAudio]=useState(false); const [reviewPlan,setReviewPlan]=useState<ReviewPlan>({});
  useEffect(()=>{fetch("/gaokao-vocab.json").then(r=>r.json()).then((d:Vocab[])=>{setWords(d);setActive(d[0]?.w||"achieve")});fetch("/audio-map.json").then(r=>r.json()).then(setAudioMap);try{setKnown(JSON.parse(localStorage.getItem("yujing-known")||"[]"));setSaved(JSON.parse(localStorage.getItem("yujing-saved")||"[]"));setReviewPlan(JSON.parse(localStorage.getItem("yujing-review")||"{}"))}catch{}},[]);
  useEffect(()=>{if(words.length){localStorage.setItem("yujing-known",JSON.stringify(known));localStorage.setItem("yujing-saved",JSON.stringify(saved));localStorage.setItem("yujing-review",JSON.stringify(reviewPlan))}},[known,saved,reviewPlan,words.length]);
  ...
  function markKnown(){if(!known.includes(active)){setKnown(k=>[...k,active]);setReviewPlan(p=>({...p,[active]:{level:0,due:Date.now(),last:Date.now()}}))}const next=words[Math.min(index+1,words.length-1)];if(next)setActive(next.w);}
  ...
  function readArticle(){speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(ARTICLES[article].text);u.lang="en-US";u.rate=.88;speechSynthesis.speak(u)}
  function finishReview(word:string,correct:boolean){const old=reviewPlan[word]?.level||0;const level=correct?Math.min(old+1,REVIEW_DAYS.length-1):0;const days=correct?REVIEW_DAYS[level]:1;setReviewPlan(p=>({...p,[word]:{level,due:Date.now()+days*86400000,last:Date.now()}}))}
  ...
  return <main className="app-shell">
    <header className="app-header"><a className="brand" href="#" onClick={()=>setView("learn")}><span>境</span><div>语境<small>高中 3500 词</small></div></a><nav>{([['learn','学习'],['reading','读文章'],['review','默写复习'],['book','生词本'],['library','全部词库']] as [View,string][]).map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}>{label}{id==="book"&&saved.length>0?<b>{saved.length}</b>:id==="review"&&dueWords.length>0?<b>{dueWords.length}</b>:null}</button>)}</nav><div className="streak">✦ 连续 12 天</div></header>
    ...
    {view==="reading"&&<section className="reader-layout"><div className="article-list">{ARTICLES.map((a,i)=><button key={a.title} className={article===i?"active":""} onClick={()=>{speechSynthesis.cancel();setArticle(i)}}><small>{a.tag}</small><strong>{a.title}</strong><span>{a.cn}</span></button>)}</div><article className="reader"><p className="eyebrow">{ARTICLES[article].tag} · 全文可点词</p><h1>{ARTICLES[article].title}</h1><h2>{ARTICLES[article].cn}</h2><button className="article-audio" onClick={readArticle}>▶ 收听全文</button><div className="article-copy"><ClickableText text={ARTICLES[article].text} map={map} onWord={selectWord}/></div><p className="reader-hint">轻触任何英文单词，即可在右侧查看并学习</p></article><StudyCard compact display={display} index={index} total={words.length} stage={stage} saved={saved.includes(active)} loading={loadingAudio} onAudio={playAudio} onSave={toggleSaved} onKnow={markKnown}/></section>}
